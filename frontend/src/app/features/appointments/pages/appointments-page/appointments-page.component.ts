    ...
    return new Promise((resolve) => {
      ...
      this.authService.searchUsersByRole('', 'CAREGIVER').subscribe({
        next: (caregivers) => {
          ...
          console.log('<svg class="inline-svg-icon text-green-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" aria-hidden="true"><path d="m5 12 4 4L19 6" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"></path></svg> Caregivers loaded:', this.myCaregivers.length);
          resolve();
        },
        error: (error) => {
    ...
    return new Promise((resolve) => {
      this.consultationTypeService.getAllConsultationTypes().subscribe({
        next: (types) => {
          ...
          console.log('<svg class="inline-svg-icon text-green-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" aria-hidden="true"><path d="m5 12 4 4L19 6" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"></path></svg> Consultation types loaded:', this.consultationTypes.length);
          resolve();
        },
        error: (error) => {
