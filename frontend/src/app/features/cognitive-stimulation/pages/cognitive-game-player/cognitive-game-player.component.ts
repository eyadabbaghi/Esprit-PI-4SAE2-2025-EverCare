...
export class CognitiveGamePlayerComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    if (!isPlatformBrowser(this.platformId)) {
      this.errorMessage = 'Chargement du jeu therapeutique dans le navigateur...';
      return;
    }
    ...
    this.route.paramMap.subscribe((params) => {
      ...
      if (!medicalRecordId || !gameId) {
        this.errorMessage = 'Les informations du jeu sont incompletes.';
        return;
      }
  ...
  private loadContext(medicalRecordId: string, gameId: string): void {
    ...
    forkJoin({
      ...
      error: (error: HttpErrorResponse) => {
        this.errorMessage = this.extractError(error, 'Impossible de charger le jeu therapeutique.');
        this.isLoading = false;
      }
    });
  }
    ...
    this.cognitiveService.createSession(this.medicalRecord.id, payload).subscribe({
      next: (session) => {
        ...
        this.successMessage = abandoned
          ? 'Le temps est ecoule. La session a ete enregistree comme inachevee.'
          : 'Le resultat du jeu a ete enregistre dans le dossier cognitif.';
        this.isSubmitting = false;
      },
      error: (error: HttpErrorResponse) => {
        this.errorMessage = this.extractError(
          error,
          abandoned ? 'Impossible d enregistrer la session apres expiration du temps.' : 'Impossible d enregistrer le resultat du jeu.'
        );
        this.isSubmitting = false;
      }
    });
  ...
  private resolveSessionNotes(abandoned: boolean): string | null {
    if (this.isInteractiveMemoryGame) {
      if (abandoned) {
        return `Session terminee automatiquement: temps ecoule apres ${this.matchedPairs}/${this.totalMemoryPairs} paires.`;
      }
      return `Memory game completed in ${this.memoryTurns} turns.`;
    }

    return abandoned ? 'Session terminee automatiquement: temps ecoule.' : null;
  }
