...
export class CognitiveGamesCatalogComponent implements OnInit {
  ...
  readonly gameTypeOptions: Array<{ value: CognitiveGameType; label: string }> = [
    ...
    { value: 'PRAXIS', label: 'Praxie' },
    { value: 'GNOSIS', label: 'Reconnaissance' },
  ];
  ...
  constructor(
    ...
    public readonly cognitiveService: CognitiveStimulationService,
  ...
  ngOnInit(): void {
    if (!isPlatformBrowser(this.platformId)) {
      this.errorMessage = 'Chargement du catalogue dans le navigateur...';
      return;
    }
  ...
  loadGames(): void {
    ...
    this.cognitiveService.listGames().subscribe({
      ...
      error: (error: HttpErrorResponse) => {
        this.errorMessage = this.extractError(error, 'Impossible de charger le catalogue des jeux.');
        this.isLoading = false;
      }
    });
  }
  ...
  applyFilters(): void {
    ...
    // Get current profession filter
    const currentProfession = this.cognitiveService.getCurrentProfession();
    const professionGames = currentProfession ? currentProfession.recommendedGames || [] : [];
    
    this.filteredGames = this.games.filter((game) => {
      const matchesProfession = professionGames.length === 0 || professionGames.includes(game.id);
      ...
      return matchesProfession && matchesSearch && matchesType && matchesDifficulty && matchesActive;
    });
  }
    ...
    request$.subscribe({
      next: () => {
        this.successMessage = this.editingGameId
          ? 'Le jeu thérapeutique a été mis à jour.'
          : 'Le jeu thérapeutique a été créé.';
        this.isSubmitting = false;
        this.resetEditor();
        this.loadGames();
      },
      error: (error: HttpErrorResponse) => {
        this.errorMessage = this.extractError(error, 'Impossible d’enregistrer le jeu thérapeutique.');
        this.isSubmitting = false;
      }
    });
  }
  ...
  deleteGame(game: CognitiveGame): void {
    if (!this.canManageGames || !window.confirm(`Supprimer le jeu "${game.title}" ?`)) {
      return;
    }
    ...
    this.cognitiveService.deleteGame(game.id).subscribe({
      next: () => {
        this.successMessage = 'Le jeu thérapeutique a été supprimé.';
      ...
      error: (error: HttpErrorResponse) => {
        this.errorMessage = this.extractError(error, 'Impossible de supprimer ce jeu.');
      }
    });
  }
