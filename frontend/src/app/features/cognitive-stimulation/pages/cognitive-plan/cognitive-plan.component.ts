...
export class CognitivePlanComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (!isPlatformBrowser(this.platformId)) {
      this.infoMessage = 'Chargement du suivi cognitif dans le navigateur...';
      return;
    }
  ...
  submitSession(): void {
    ...
    this.cognitiveService.createSession(this.currentRecord.id, payload).subscribe({
      next: () => {
        this.successMessage = 'La session cognitive a bien été enregistrée.';
        this.isSubmitting = false;
        this.resetSessionForm();
        this.loadCognitiveData(this.currentRecord!.id);
      },
      error: (error: HttpErrorResponse) => {
        this.errorMessage = this.extractError(error, 'Impossible d’enregistrer la session.');
        this.isSubmitting = false;
      }
    });
  }
    ...
    this.cognitiveService.deleteSession(session.id).subscribe({
      next: () => {
        if (this.currentRecord) {
          this.successMessage = 'La session a été supprimée.';
          this.loadCognitiveData(this.currentRecord.id);
        }
      },
      error: (error: HttpErrorResponse) => {
        this.errorMessage = this.extractError(error, 'Impossible de supprimer la session.');
      }
    });
  }
  ...
  private bootstrapView(): void {
    ...
    this.sessions = [];
    this.infoMessage = 'Ouvrez cette vue depuis un dossier médical pour consulter la progression cognitive d’un patient.';
  }
    ...
    this.medicalRecordService.getById(recordId).subscribe({
      next: (record) => {
        if (this.currentRole === 'PATIENT' && !this.isOwnRecord(record)) {
          ...
          this.errorMessage = 'Vous ne pouvez consulter que votre propre suivi cognitif.';
          return;
        }
      ...
      error: (error: HttpErrorResponse) => {
        this.currentRecord = null;
        this.isLoading = false;
        this.errorMessage = this.extractError(error, 'Impossible de charger le dossier médical lié.');
      }
    });
  }
  ...
  private tryLoadOwnRecordCandidate(index: number, hasTechnicalError: boolean): void {
    if (index >= this.patientIdentifierCandidates.length) {
      ...
      this.infoMessage = hasTechnicalError
        ? ''
        : 'Aucun dossier médical n’est encore associé à votre compte.';
  ...
  private loadCognitiveData(medicalRecordId: string): void {
    forkJoin({
      ...
      error: (error: HttpErrorResponse) => {
        this.isLoading = false;
        this.errorMessage = this.extractError(error, 'Impossible de charger les données de stimulation cognitive.');
      }
    });
  }
